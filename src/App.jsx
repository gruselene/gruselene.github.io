...
const Layout = ({ children }) => {
  ...
  // 页面跳转时只对前进导航滚动到顶部，回退保持浏览器默认行为
  React.useEffect(() => {
    // 使用 popstate 事件检测浏览器后退/前进
    const handlePopState = () => {
      // 浏览器后退时，不做任何处理，让浏览器自然恢复位置
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-mystic-950 text-mystic-100 selection:bg-gold-500 selection:text-mystic-950 relative overflow-x-hidden">
      ...
      {/* Main Content */}
      <main className="flex-grow max-w-5xl mx-auto px-6 sm:px-8 py-12 w-full relative">
