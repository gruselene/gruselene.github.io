...
const MyWanderersLibrary = () => {
  const category = getCategory('external', 'myWanderersLibrary');
  const wanderersWorks = category ? sortByDate(category.works || []) : [];
    ...
    <div className="space-y-16">
      ...
      <section className="text-center space-y-8">
        <h1 className="text-4xl font-serif font-bold tracking-[0.3em] uppercase bg-gradient-to-r from-gold-400 to-white bg-clip-text text-transparent">
          {category?.title || '被放逐者图书馆原创和翻译'}
        </h1>
        <p className="text-mystic-300 font-serif italic opacity-70">
          {category?.description || 'AStarTwoEyes在被放逐者图书馆的原创和翻译合集'}
        </p>
      </section>

      {/* Wanderers Library Works List */}
      <section className="space-y-8">
        {renderSingleColumn(wanderersWorks, (work) => (
...
export default MyWanderersLibrary;
