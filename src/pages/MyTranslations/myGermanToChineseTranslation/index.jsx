...
const MyGermanToChineseTranslation = () => {
  // 自动注册小类和作品
  useEffect(() => {
    // 注册小类
    ...
    // 注册作品
    autoRegisterWork('translations', 'myGermanToChineseTranslation', {
      id: 'Work1',
      title: 'Placeholder German to Chinese Translation 1',
      excerpt: 'Original German text goes here...',
      date: '2024-04-27',
  ...
  // Find the myGermanToChineseTranslation category
  const category = getCategory('translations', 'myGermanToChineseTranslation');
          ...
          <article className="mystic-card group">
            ...
            {work.title && (
              <h2 className="card-title">
                <Link to={`/my-translations/myGermanToChineseTranslation/${work.id}`}>{work.title}</Link>
              </h2>
            )}
            {work.excerpt && (
...
export default MyGermanToChineseTranslation;